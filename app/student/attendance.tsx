...
const StudentAttendanceScreen = () => {
  ...
  const loadAttendanceForSubject = async (subjectId: string) => {
    ...
    try {
      ...
      studentAttendance.sort((a, b) => (b.date || '').localeCompare(a.date || ''));
      ...
      <ScrollContainer contentClassName="px-4 sm:px-6 pt-12 pb-10 gap-4">
        <View className="flex-row items-center justify-between mb-4">
          <View>
            ...
            <Text className="text-gray-500 mt-1">View your attendance records by subject</Text>
          </View>
        </View>
        ...
        {subjects.length > 0 && (
          <GlassCard className="p-4">
            <Text className="text-sm font-semibold text-gray-700 mb-3">Select Subject</Text>
                ...
                >
                  <Text
                    className={`font-semibold ${
                      selectedSubject?.id === subject.id ? 'text-white' : 'text-gray-700'
                    }`}
                  >
                    {subject.name}
                  </Text>
        ...
        {selectedSubject && (
          <GlassCard className="p-4">
            ...
            {attendanceRecords.length === 0 ? (
              <Text className="text-center text-gray-500 py-8">No attendance records found</Text>
            ) : (
              <View className="space-y-2">
                {/* Table Header */}
                <View className="flex-row bg-gray-100 rounded-lg p-3 mb-2">
                  <Text className="flex-1 font-semibold text-gray-700">Date</Text>
                  <Text className="flex-1 font-semibold text-gray-700 text-right">Status</Text>
                </View>
                {/* Table Rows */}
                      ...
                      <View className="flex-row items-center gap-2">
                        <View
                          className={`w-3 h-3 rounded-full ${
                            status === 'present' ? 'bg-green-500' : 'bg-rose-500'
                          }`}
                        />
                        <Text
                          className={`font-semibold ${
                            status === 'present' ? 'text-green-700' : 'text-rose-700'
                          }`}
                        >
